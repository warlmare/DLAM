...
class HashDataset(Dataset):
    ...
    def __init__(self, hashes, labels, all_chars, hashing_algorithm='SSDEEP', transform=False):
        """
        .
        """
...
anomalies_path = "evaluation_testcase_random/training_data_for_model/anomaly_hashes_50000_singlefragment_1-99_ssdeep_random.csv"
normal_path = "evaluation_testcase_random/training_data_for_model/normal_hashes_50000_singlefragment_1-99_ssdeep_random.csv"
...
print("consistency check| train_data: ",len(training_data_list))
print("consistency check| train_labels: ",len(training_labels))
...
print("consistency check| val_data: ", len(val_data_list))
print("consistency check| val_labels: ", len(val_labels))
...
# test_anomalies_path = "dataset/anomaly_hashes_50000_singlefragment_1-99_tlsh_javascript.csv"
# test_normal_path = "dataset/normal_hashes_50000_singlefragment_1-99_tlsh_javascript.csv"
...
max_steps =  1250 #1250# 250#  epochs #585 # 6 epochs 1500 # 1230 3 epochs for 210000 training data and 1024 batch size
...
class SimpleClassificationModel(nn.Module):
    def __init__(self, input_length, embedding_size=32, hidden_units_1=256, hidden_units_2=64, 
                 dropout_prob=0.125):
...
# Example useage
# input_x , input_y = next(train_loader_generator)
...
# net = SimpleClassificationModel(input_length=96) #72
# out = net(input_x)
# out.shape
...
elif hashing_algorithm == "ssdeep":
  # ssdeep ist ein variabel langer hash, nun sollen alle inputs auf die selbe länge gepadded werden. 
  # hier wird die funktion gerufen die auf die feste länge 96 padded. 
  train_data_loader = DataLoader(train_dataset, batch_size, collate_fn=ssdeep_collate_fn,  shuffle=True, num_workers=2)
  val_data_loader = DataLoader(val_dataset, batch_size, collate_fn=ssdeep_collate_fn,  shuffle=True, num_workers=2)
else:
...
for step in range(max_steps):
  ...
  # VALIDATION ACCURACY
    target = F.sigmoid(val_prediction_logits)
    round_predictions = torch.round(target)
    flat_predictions = torch.flatten(round_predictions)
    ...
    correct_predictions = (val_batch_labels == flat_predictions).sum().cpu().numpy()
...
print('Done training.')
torch.save(model, "evaluation_testcase_random/trained_feedforward_model_{}.pth".format(hashing_algorithm))
...
model = torch.load("evaluation_testcase_random/trained_feedforward_model_{}.pth".format(hashing_algorithm))
model.eval()
...
testhash = "98304:ILFINWu3oh9DBUyepNaEejVSm6+V5EuD2KSQRCjMyai:IZ253o7DBfepNaN760lRQMm,24576:JesOWiibbhkMJ9D7AsNSH0UyuthoT6KzO24No8m:J7OIh9D8sNSH0NoW+mOf28m"

embedding_1 = hash_to_embedding("SSDEEP",testhash,1)
#embedding_colated = ssdeep_collate_fn(embedding_1)
#print(embedding_colated)
...
print(single_input_prediction(embedding_1[0],model,device))
